...
| `agent context` | 已实现 | 输出当前项目、backend、安全约束和 catalog 摘要 |
| `cache build/status/clear/save-semantic/get-semantic` | 已实现 | 覆盖元数据缓存与语义缓存 |
| `@natural` | 规划中 | 未实现 |
| `agent plan` / `agent run` | 已移除 | 当前工作树不再暴露这些命令 |
| `skill list/info` / `agent skill` | 已移除 | Skill 文档改为外部 Agent 直接读取 `.maxc/skills/*.md` |
...
- `backend.type=auto`：
  - 如果环境变量或配置文件中存在完整连接信息，则走 `odps`
  - 否则回退到 `mock`
- `backend.type=odps|maxcompute`：
  - 强制走真实 backend
- `backend.type=mock`：
  - 强制走本地 mock
...
- `identity_source=environment | config_file | mixed`
- `principal_display` 为 access_id 脱敏值
...
- Skill 文档由外部 Agent 直接读取 `.maxc/skills/*.md`
