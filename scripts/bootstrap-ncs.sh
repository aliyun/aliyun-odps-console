...
if command -v ncs &> /dev/null; then
    ...
    OS=$(uname -s)
    ARCH=$(uname -m)
    echo -e "  检测到系统: ${YELLOW}${OS} ${ARCH}${NC}"
    ...
    if [ "$OS" = "Darwin" ]; then
        if [ "$ARCH" = "arm64" ]; then
            ...
            echo -e "  ${RED}macOS 仅支持 arm64 和 x86_64${NC}"
            exit 1
        fi
    elif [ "$OS" = "Linux" ]; then
        if [ "$ARCH" = "aarch64" ]; then
            NCS_URL="https://authx.alibaba-inc.com/api/v1/releases/latest/bin/linux/arm64/ncs"
        elif [ "$ARCH" = "x86_64" ]; then
            NCS_URL="https://authx.alibaba-inc.com/api/v1/releases/latest/bin/linux/amd64/ncs"
        else
            echo -e "  ${RED}不支持的架构: ${ARCH}${NC}"
            echo -e "  ${RED}Linux 仅支持 aarch64 和 x86_64${NC}"
            exit 1
        fi
    else
        echo -e "  ${RED}不支持的操作系统: ${OS}${NC}"
        echo -e "  ${RED}仅支持 macOS (Darwin) 和 Linux${NC}"
        exit 1
    fi
    ...
    if [ "$(uname)" = "Darwin" ]; then
        xattr -c "$NCS_TEMP" 2>/dev/null || true
    fi
