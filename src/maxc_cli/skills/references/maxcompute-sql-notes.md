...
Read this file for `maxc`-specific SQL behaviors: how `maxc query` injects SET options, how the read-only gate works, how result fetching behaves, and how `maxc data upload` maps to `INSERT INTO` / `INSERT OVERWRITE` semantics.
...
The gate applies to SQL only. **`maxc data upload` is not gated** because it goes through the Tunnel API (a write path by design) — see "Upload semantics" below.
...
## INSERT Semantics ↔ `maxc data upload`

Understanding write semantics helps interpret data patterns and pick the right upload mode:

| SQL statement | Effect | `maxc data upload` equivalent |
|---|---|---|
| `INSERT INTO` | Append rows to the table/partition | `maxc data upload <table> --file path.csv [--partition ...]` (default append) |
| `INSERT OVERWRITE` | Replace all data in the target table/partition | `maxc data upload <table> --file path.csv [--partition ...] --overwrite` |
...
Note: `maxc data upload` goes through Tunnel (no SQL CU consumed), supports primitive types only (no array/map/struct), is fail-fast on bad rows, and requires the target table to already exist. For very large or parallel transfers, use `odpscmd tunnel` with multiple threads.
